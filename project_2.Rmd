...
postResample(pred = pls_pred, obs = test_y_high_corr)
```
...
# postResample(pred = nnet_pred, obs = test_y)
```
...
postResample(pred = mars_pred, obs = test_y)
```
...
postResample(pred = svmR_pred, obs = test_y %>% as_vector())
```
...
postResample(pred = knn_pred, obs = test_y %>% as_vector())
```
...
postResample(pred = single_trees_pred, obs = test_y)
```
...
postResample(pred = tree_pred, obs = test_y)
```
...
postResample(pred = bagged_tree_pred, obs = test_y)
```
...
postResample(pred = gbm_pred, obs = test_y)
```
...
postResample(pred = cubist_pred, obs = test_y)
```
...
update_tracker <- function(model_name, actual, predicted){
  model_metrics <- postResample(pred = predicted, obs = actual)
